...
public class TCSolver {
    int solve(String fileNameGrammar, String fileNameGraph, String fileout) throws FileNotFoundException {
        GrammarCNF grammar = parseGrammar(fileNameGrammar);
        ...
        int count = 0, countAll = 0, last_countAll = 0;
        while (true) {
            ...
            for (int i = 0; i < size; i++) {
                for (int j = 0; j < size; j++) {
                    ...
                    countAll += matrix[i][j].size();
                }
            }
            if (last_countAll == countAll) {
                break;
            }
            last_countAll = countAll;
        }
        PrintWriter pw = null;
...
//            pw = new PrintWriter(System.out, true);
    ...
    GrammarCNF parseGrammar(String filename) throws FileNotFoundException {
        Scanner sc = new Scanner(new File(filename));
        GrammarCNF grammar = new GrammarCNF();
